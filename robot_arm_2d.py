...
class RobotArm2d():
    ...
    """
    def __init__(self, lengths: list = [0.5, 0.5, 1], sigmas: list = [0.25, 0.5, 0.5, 0.5], viz_dir: str = "prior"):
    ...
    def distance_euclidean(self, pos_target: torch.FloatTensor, pos: torch.FloatTensor) -> Tuple[float, float]:
        """
        Calculate the pairwise L2 distance between each predicted position and its respective target,
        returning the mean and standard deviation.

        :param pos_target: Target end effector positions, shape (n, 2)
        :param pos: Predicted end effector positions, shape (n, 2)
        :return: Tuple of (mean distance, std deviation)
        """
        pdist = torch.nn.PairwiseDistance(p=2)
        pos = pos.to(pos_target.device)
        distances = pdist(pos_target, pos)  # shape (n,)
        mean_dist = torch.mean(distances)
        std_dist = torch.std(distances)
        return mean_dist.item(), std_dist.item()
    ...
    def init_plot(self) -> plt.figure:
        ...
        return plt.subplots(figsize=(8.75, 8.75))
    ...
    def viz_inverse(self, pos: torch.FloatTensor, thetas: torch.FloatTensor, save: bool = True, show: bool = False, fig_name: str = "fig_inverse", viz_format: tuple = (".png"), ax: plt.axes = None) -> Tuple[plt.axes, float]:
        ...
        plt.rcParams['font.serif'] = ['CMU Serif', 'DejaVu Serif', 'Times New Roman', 'serif']
        ...
        distance,std = self.distance_euclidean(pos, p_next)

        # Plot cross to mark end effector position
        l_cross = 1
        pos = pos.to('cpu')
        ax.vlines(pos[:, 0], pos[:, 1]-l_cross, pos[:, 1]+l_cross, ls='-', colors='black', linewidth=.73, alpha=.5, zorder=-1)
        ax.hlines(pos[:, 1], pos[:, 0]-l_cross, pos[:, 0]+l_cross, ls='-', colors='black', linewidth=.73, alpha=.5, zorder=-1)
        ...
        ax.tick_params(axis='both', labelsize = 30)  # Increase tick font size

        # If ax were passed we can only save the subplot and not the individual plots
        if save and not passed_ax:
            plt.savefig(os.path.join(self.viz_dir, fig_name) + '.png')
        if show:
            plt.show()
        if passed_ax:
            return ax, distance, std
        else:
            plt.close(fig)
            return ax, distance, std
